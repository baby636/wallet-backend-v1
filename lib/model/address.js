...
var bchjs = require('bitcoincashjs');
...
Address.create = function(opts) {
  ...
  x.address = convertToLegacy(opts.address);
...
Address.fromObj = function(obj) {
  ...
  x.address = convertToLegacy(obj.address);
...
Address._deriveAddress = function(scriptType, publicKeyRing, path, m, coin, network) {
  ...
  return {
    address: convertToLegacy(bitcoreAddress.toString()),
    path: path,
...
function convertToLegacy(address) {
  return bchjs.Address.fromString(address).toString();
}
